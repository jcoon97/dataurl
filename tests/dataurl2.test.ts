...
describe("DataURL2 Tests", () => {
    test("Parse (PNG Image)", () => {
        const rawResponse: ParsedData | false = new DataUrl2().parse(TEST_DATA_URL);
    ...
    test("Parse (HTML, Un-encoded)", () => {
        ...
        const rawResponse: ParsedData | false = new DataUrl2().parse(rawDataUrl);
        expect(rawResponse).not.toBe(false);
    ...
    test("Convert/Stringify (PNG Image)", () => {
        const result: string = new DataUrl2().convert({
            data: TEST_FILE,
            mimetype: "image/png"
        });
