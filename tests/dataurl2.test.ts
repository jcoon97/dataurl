...
import { convertDataUrl, parseDataUrl, ParsedData } from "../lib";
...
describe("DataURL2 Tests", () => {
    test("Parse (PNG Image)", () => {
        const rawResponse: ParsedData | false = parseDataUrl(TEST_DATA_URL);
    ...
    test("Parse (HTML, Un-encoded)", () => {
        ...
        const rawResponse: ParsedData | false = parseDataUrl(rawDataUrl);
        expect(rawResponse).not.toBe(false);
    ...
    test("Convert/Stringify (PNG Image)", () => {
        const result: string = convertDataUrl({
            data: TEST_FILE,
            mimetype: "image/png"
        });
